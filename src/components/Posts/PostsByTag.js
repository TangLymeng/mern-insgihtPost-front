...
import { fetchPostsByTagAction } from "../../redux/slices/posts/postsSlice";
...
const PostsByTag = () => {
  ...
  useEffect(() => {
    const tag = location.state?.tagName || tagName;
    if (tag) {
      dispatch(fetchPostsByTagAction(tag));
    }
  }, [dispatch, location.state, tagName]);
  ...
  const renderPosts = () => {
    ...
    if (error) {
      return <h3 className="text-red-500 text-center">{error.message}</h3>;
    }
    ...
    return posts.map((post) => (
      <div key={post._id} className="w-full md:w-1/2 px-4 mb-8">
        <a className="block mb-6 overflow-hidden rounded-md" href="#">
          <img className="w-full h-80 object-cover" alt="post image" src={post.image} />
        </a>
        <div className="mb-4">
          <a
